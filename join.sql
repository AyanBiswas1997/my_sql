...
drop table student
create table marks(english_marks int,
math_marks int,
id int,
foreign key (id) references student(id)
);
drop table marks
insert into student values(1,"Ayan","Biswas",6),(2,"Rahul","Biswas",9),(6,"Ayan","nath",8),(7,"mampi","Roy",9),(3,"Raja","banarjee",10);
insert into marks values(57,69,1);
select * from student as st inner join marks as mk on st.id=mk.id;
select * from student as st left join marks as mk on st.id=mk.id;
order by class;

select * from student as st right join marks as mk on st.id=mk.id;

select * from student as st right outer join marks as mk on st.id=mk.id;
select * from student as st  left outer join marks as mk on st.id=mk.id;
order by desc class;
